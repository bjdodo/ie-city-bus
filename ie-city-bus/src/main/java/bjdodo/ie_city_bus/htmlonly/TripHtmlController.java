...
public class TripHtmlController {
	...
	public String getTripPassages(@PathVariable Long tripId) {
		...
		List<ActiveTrip> activeTrip = activeTripsRepository.getActiveTripsById(new long[] { tripId });
		if (activeTrip.size() > 0) {
			routeShortName = activeTrip.get(0).getRouteShortName();
		}

		return getFormattedHtml(stopPassageDetails, routeShortName);
	}
	
	private String getFormattedHtml(List<StopPassageDetail> stopPassageDetails, String routeShortName) {
		
		return HtmlUtils.getStopPassageDetailsHtml(stopPassageDetails, routeShortName);
	}
}
