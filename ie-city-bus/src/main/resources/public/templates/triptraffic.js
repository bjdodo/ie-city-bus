angular.module('myApp').controller('TripTrafficController', function($scope, httpwrappersvc, $interval, $routeParams) {
	...
	$scope.updateData = function() {
		var promiseTrip = httpwrappersvc.httpGet('api/trip/' + $scope.tripId + "/traffic");
		promiseTrip.then(function(response) {
			...
			for (stopsIdx = 0; stopsIdx < $scope.tripTraffic.length; stopsIdx++) {
				...
				// passages are ordered by time and we are looking for the most recent ones
				...
				for (passageIdx = passageInfo.tripSectionPassages.length-1; passageIdx >= 0; passageIdx--) {
					...
					currentEntry["duration" + cnt] = passageInfo.tripSectionPassages[passageIdx].stop2Time - passageInfo.tripSectionPassages[passageIdx].stop1Time;
				...
				$scope.tripTrafficFormatted.push(currentEntry);
			}
