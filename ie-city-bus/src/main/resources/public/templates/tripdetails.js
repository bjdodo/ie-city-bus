angular.module('myApp').controller(
		...
		function($scope, httpwrappersvc, $interval, $routeParams) {
			...
			$scope.updateData = function() {
				var promiseTrip = httpwrappersvc.httpGet('api/activetrip/' + $scope.tripId);
				promiseTrip.then(function(response) {
					...
					if ($scope.tripData != null) {
						...
						$scope.mapData.pins = [ {
							latitude : latlong.latitude,
							longitude : latlong.longitude,
							description : '<div class="mappinpopup"><b>' + $scope.tripData.routeShortName + '</b><br/>' + 'Destination: '
									+ $scope.tripData.destinationStopName + '</div>',
							pngFile : 'img/bus.png'
						} ];
				...
				var promisePassages = httpwrappersvc.httpGet('api/trip/' + $scope.tripId + '/passages');
