...
--from
--(select max(actual_arrival) actual_arrival, trip_id from stop_passage where  actual_arrival<CURRENT_TIMESTAMP() and trip_id in (select current_trip_id from vehicle where current_trip_id is not null) group by trip_id) current_stop_passage
